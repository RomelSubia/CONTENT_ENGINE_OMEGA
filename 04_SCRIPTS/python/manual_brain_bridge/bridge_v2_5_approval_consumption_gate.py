...
LAYER = "BRIDGE_APPROVAL_CONSUMPTION_GATE_LAYER_V2_5"
...
def build_repo_identity_report(
    root: Path,
    head: str,
    branch: str,
    remote: str,
...
def build_contamination_gate_report(root: Path, repo_identity: dict[str, Any]) -> dict[str, Any]:
    ...
    report = base_report(
        "BRIDGE_V2_5_ARGOS_CONTAMINATION_GATE_REPORT",
...
def build_v24_authority_consumption_report(root: Path) -> dict[str, Any]:
    ...
    for authority_id, rel in V24_AUTHORITY_FILES.items():
        ...
        if authority_id == "v24_human_approval_gate_seal":
            ...
            ok = (
                data.get("status") == "PASS"
                and data.get("contamination_detected") is False
            )
        else:
            ok = False
...
def resolve_approval_attempt(
    ...
    normalized_text = (approval_text or "").strip()
    ...
    blocked_sources = {
        ...
        "BRAIN": "BLOCK_APPROVAL_FROM_BRAIN",
    ...
    if normalized_text.lower() in generic_approvals:
        return {
            "allowed": False,
    ...
    if normalized_source != "FRESH_HUMAN_TERMINAL_INPUT":
        return {
            "allowed": False,
    ...
    approval_hash = sha256_text(normalized_text)

    if approval_hash != expected_hash:
        return {
            "allowed": False,
            "decision": "BLOCK_APPROVAL_HASH_MISMATCH",
            "approval_granted": False,
            "approval_consumed": False,
            "build_allowed_next": False,
            "build_allowed_now": False,
    ...
    if not head_matches:
        return {
            "allowed": False,
            "decision": "BLOCK_APPROVAL_FROM_OLD_HEAD",
            "approval_granted": False,
            "approval_consumed": False,
            "build_allowed_next": False,
            "build_allowed_now": False,
    ...
    if not authority_matches:
        return {
            "allowed": False,
            "decision": "BLOCK_APPROVAL_FROM_WRONG_AUTHORITY",
            "approval_granted": False,
            "approval_consumed": False,
            "build_allowed_next": False,
            "build_allowed_now": False,
    ...
    if not scope_matches:
        return {
            "allowed": False,
            "decision": "BLOCK_APPROVAL_FROM_WRONG_SCOPE",
            "approval_granted": False,
            "approval_consumed": False,
            "build_allowed_next": False,
            "build_allowed_now": False,
    ...
    return {
        "allowed": True,
        "decision": "ALLOW_APPROVAL_CONSUMPTION",
...
def build_approval_input_receipt_report(approval_text: str, challenge: dict[str, Any]) -> dict[str, Any]:
    expected_hash = str(challenge.get("expected_approval_hash"))
    attempt = resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", approval_text, expected_hash)

    input_present = bool((approval_text or "").strip())
    approval_input_hash = sha256_text((approval_text or "").strip()) if input_present else None
    match = approval_input_hash == expected_hash if approval_input_hash else False
    ok = attempt.get("allowed") is True and match is True

    report = base_report("BRIDGE_V2_5_APPROVAL_INPUT_RECEIPT_REPORT", "PASS" if ok else "BLOCK")
    report.update({
        "human_approval_required": True,
        "human_approval_received": input_present,
        "approval_source": "FRESH_HUMAN_TERMINAL_INPUT",
        "approval_source_allowed": True,
        "approval_plaintext_storage_allowed": False,
        "approval_plaintext_stored": False,
        "approval_input_hash_present": approval_input_hash is not None,
        "approval_input_hash": approval_input_hash,
        "expected_approval_hash": expected_hash,
        "approval_match": match,
        "approval_attempt_decision": attempt.get("decision"),
        "approval_granted": False,
        "approval_consumed": False,
        "build_allowed_next": False,
        "build_allowed_now": False,
...
def build_approval_binding_report(
...
def build_approval_consumption_report(
    receipt: dict[str, Any],
...
def build_anti_replay_consumption_report(challenge: dict[str, Any]) -> dict[str, Any]:
    expected_hash = str(challenge.get("expected_approval_hash"))
    phrase = str(challenge.get("required_approval_phrase"))

    blocked_cases = [
        resolve_approval_attempt("HISTORY", phrase, expected_hash),
        resolve_approval_attempt("REPORT", phrase, expected_hash),
        resolve_approval_attempt("COMMIT", phrase, expected_hash),
        resolve_approval_attempt("MANUAL", phrase, expected_hash),
        resolve_approval_attempt("BRAIN", phrase, expected_hash),
        resolve_approval_attempt("OLD_SEAL", phrase, expected_hash),
        resolve_approval_attempt("PREVIOUS_LAYER", phrase, expected_hash),
        resolve_approval_attempt("GLOBAL", phrase, expected_hash),
        resolve_approval_attempt("SELF", phrase, expected_hash),
        resolve_approval_attempt("NONE", "ok", expected_hash),
        resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", "ok", expected_hash),
        resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", phrase, expected_hash, already_consumed=True),
        resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", phrase, expected_hash, head_matches=False),
        resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", phrase, expected_hash, authority_matches=False),
        resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", phrase, expected_hash, scope_matches=False),
    ]

    valid_case = resolve_approval_attempt("FRESH_HUMAN_TERMINAL_INPUT", phrase, expected_hash)
    ok = all(case.get("allowed") is False for case in blocked_cases) and valid_case.get("allowed") is True
    ...
    report.update({
        ...
        "valid_case_allowed": valid_case.get("allowed") is True,
        "approval_plaintext_stored": False,
        "build_allowed_now": False,
    })
    return report
...
def build_blocked_capabilities_report(consumption: dict[str, Any]) -> dict[str, Any]:
    ...
    report.update({
        ...
        "build_allowed_now": False,
        "execution_allowed": False,
        "policy": "v2.5 consumes approval only; it does not execute the next governed build.",
    })
    return report
...
def build_human_report(validation: dict[str, Any], receipt: dict[str, Any], consumption: dict[str, Any]) -> str:
    return f"""# BRIDGE v2.5 APPROVAL CONSUMPTION GATE — MANUAL ↔ CEREBRO
...
def generate(
    ...
    receipt = build_approval_input_receipt_report(approval_text, challenge)
    ...
    output_map = {
        ...
        "00_SYSTEM/bridge/reports/BRIDGE_V2_5_ARGOS_CONTAMINATION_GATE_REPORT.json": contamination,
    ...
    seal_status = (
        "SEALED_AS_APPROVAL_CONSUMPTION_GATE_V2_5"
    ...
    return EXIT_PASS if seal_status == "SEALED_AS_APPROVAL_CONSUMPTION_GATE_V2_5" else EXIT_BLOCK
...
def validate_outputs(root: Path) -> int:
    ...
    contamination = read_json(root, "00_SYSTEM/bridge/reports/BRIDGE_V2_5_ARGOS_CONTAMINATION_GATE_REPORT.json")

    checks = {
        "validation_pass": validation.get("status") == "PASS",
        "manifest_pass": manifest.get("status") == "PASS",
        "seal_status": seal.get("status") == "SEALED_AS_APPROVAL_CONSUMPTION_GATE_V2_5",
...
def main() -> int:
    ...
    if args.mode == "generate":
        ...
        approval_text = sys.stdin.read().strip()
