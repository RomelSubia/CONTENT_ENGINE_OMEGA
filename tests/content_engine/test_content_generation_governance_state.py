...
from content_engine.content_generation_governance import build_content_generation_governance_state, validate_content_generation_governance_state

def test_state_is_governance_only_and_hard_false():
    state = build_content_generation_governance_state()
    assert state["layer_mode"] == "GOVERNANCE_ONLY"
    assert state["human_review_required"] is True
    assert validate_content_generation_governance_state(state)["status"] == "PASS"
