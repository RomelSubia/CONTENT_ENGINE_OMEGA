...
from content_engine.content_generation_governance import make_generation_output, validate_generation_output_schema

def test_blocked_outputs_still_require_human_review():
...
def test_low_review_block_never_allows_generation():
    output = make_generation_output(
        status="PASS",
        generation_governance_status="GENERATION_HUMAN_REVIEW_REQUIRED",
        reason="low risk still review",
        risk_level="LOW_REVIEW_BLOCK",
    )
    assert output["content_generation_allowed_now"] is False
    assert output["draft_creation_allowed_now"] is False
