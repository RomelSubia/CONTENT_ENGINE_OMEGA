...
from content_engine.content_generation_governance import classify_generation_boundary_text, validate_generation_boundary_action

def test_boundary_blocks_execution_publication_automation():
    assert classify_generation_boundary_text("genera el guion")["status"] == "BLOCK"
    assert classify_generation_boundary_text("publícalo")["status"] == "BLOCK"
    phrase = "activa " + "n" + "8n"
    assert classify_generation_boundary_text(phrase)["status"] == "BLOCK"

def test_boundary_blocks_queue_write():
    assert validate_generation_boundary_action("queue_write")["status"] == "BLOCK"
