...
from content_engine.content_draft_finalization_governance.policy import (
    FinalizationPolicy,
    FinalizationPolicyError,
)

def test_policy_accepts_allowed_decision():
    assert FinalizationPolicy.assert_allowed_decision(
        "FINALIZE_FOR_QUEUE_GOVERNANCE_PLAN_ONLY"
    ) == FinalizationDecision.FINALIZE_FOR_QUEUE_GOVERNANCE_PLAN_ONLY

def test_policy_rejects_forbidden_decision():
    with pytest.raises(FinalizationPolicyError):
        FinalizationPolicy.assert_allowed_decision("PUBLISH_NOW")
