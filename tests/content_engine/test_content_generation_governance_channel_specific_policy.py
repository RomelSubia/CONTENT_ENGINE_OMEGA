...
from content_engine.content_generation_governance import validate_channel_specific_policy

def test_channel_specific_policy_matrix_blocks():
    cases = [
        ("CHANNEL_A_MONEY_MINDSET_CONVERSION", "dinero garantizado"),
        ("CHANNEL_D_AI_TECH_PERSONAL_BRAND", "motivación genérica"),
        ("CHANNEL_B_CURIOSITIES_MASS_TRAFFIC", "hecho no verificable"),
        ("CHANNEL_C_MOTIVATION_DISCIPLINE_RETENTION", "disciplina tóxica"),
    ]
    for channel, summary in cases:
        candidate = safe_candidate()
        candidate["channel_id"] = channel
        candidate["idea_summary"] = summary
        assert validate_channel_specific_policy(candidate)["status"] == "BLOCK"
