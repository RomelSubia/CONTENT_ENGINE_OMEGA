...
from content_engine.content_generation_governance import validate_generation_traceability

def test_traceability_requires_closed_chain():
    assert validate_generation_traceability(safe_candidate())["status"] == "PASS"
    candidate = safe_candidate()
    candidate["traceability"]["prompt_governance_ref"] = "DEFINED"
    assert validate_generation_traceability(candidate)["status"] == "BLOCK"
