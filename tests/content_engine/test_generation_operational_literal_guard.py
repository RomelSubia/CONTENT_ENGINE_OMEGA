...
from content_engine.content_generation_governance import validate_no_operational_literals

def test_forbidden_operational_literals_are_blocked():
    literal = "OPEN" + "AI_API_KEY"
    assert validate_no_operational_literals(literal)["status"] == "BLOCK"
    assert validate_no_operational_literals("safe governance text")["status"] == "PASS"
