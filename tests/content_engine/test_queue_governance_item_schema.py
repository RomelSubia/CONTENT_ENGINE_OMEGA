from __future__ import annotations

import sys
from pathlib import Path

ROOT = Path(__file__).resolve().parents[2]
SRC = ROOT / "04_SCRIPTS" / "python"
if str(SRC) not in sys.path:
    sys.path.insert(0, str(SRC))

from content_engine import queue_governance as qg


def safe_item():
    return {
        "schema_version": "QUEUE_ITEM_SCHEMA_V1",
        "queue_item_id": "QG-20260516-231455-A13F09BC",
        "created_at_utc": "2026-05-16T23:14:55Z",
        "source_type": "MANUAL_IDEA",
        "channel_id": "CHANNEL_D_AI_TECH_PERSONAL_BRAND",
        "content_intent": "education",
        "idea_title": "Automatización con IA para productividad",
        "idea_summary": "Idea conceptual para revisión humana sin publicación",
        "pillar_id": "PILLAR_AI_AUTOMATION",
        "audience_profile_id": "AUDIENCE_TECH_BUILDERS",
        "lifecycle_state": "DRAFT_INTAKE",
        "priority_score": 0,
        "priority_band": "LOW",
        "readiness_score": 0,
        "readiness_status": "NOT_READY",
        "risk_level": "LOW_REVIEW_BLOCK",
        "evidence_required": True,
        "evidence_status": "PARTIAL",
        "generation_allowed": False,
        "publishing_allowed": False,
        "queue_write_allowed": False,
        "blocked_reason": None,
        "traceability": {
            "construction_core_ref": "CLOSED_VALIDATED",
            "strategy_foundation_ref": "CLOSED_VALIDATED",
            "prompt_governance_ref": "CLOSED_VALIDATED",
            "queue_governance_ref": "QUEUE_GOVERNANCE_CORE",
        },
    }


def test_valid_item_schema_passes():
    assert qg.validate_queue_item_schema(safe_item())["status"] == "PASS"


def test_conceptual_item_build_passes_without_real_queue():
    result = qg.build_conceptual_queue_item(safe_item())
    assert result["status"] == "PASS"
    assert result["real_queue_item"] is None
    assert result["queue_write_performed"] is False


def test_missing_schema_version_blocks():
    item = safe_item()
    del item["schema_version"]
    assert qg.validate_queue_item_schema(item)["reason"] == "QUEUE_SCHEMA_VERSION_MISSING"


def test_invalid_schema_version_blocks():
    item = safe_item()
    item["schema_version"] = "BAD"
    assert qg.validate_queue_item_schema(item)["reason"] == "QUEUE_SCHEMA_VERSION_INVALID"


def test_generation_allowed_true_blocks():
    item = safe_item()
    item["generation_allowed"] = True
    assert qg.validate_queue_item_schema(item)["status"] == "BLOCK"


def test_publishing_allowed_true_blocks():
    item = safe_item()
    item["publishing_allowed"] = True
    assert qg.validate_queue_item_schema(item)["status"] == "BLOCK"


def test_queue_write_allowed_true_blocks():
    item = safe_item()
    item["queue_write_allowed"] = True
    assert qg.validate_queue_item_schema(item)["status"] == "BLOCK"

def _make_invalid_id_blocks_test(value):
    def _test():
        result = qg.validate_queue_item_id(value)
        assert result["status"] == "BLOCK"
        assert result.get("queue_write_performed", False) is False
    return _test


for _idx, _value in enumerate(['bad', 'QG-20260516-231455-ZZZZZZZZ', 'QG-20260516-231455-A13F09BC/path', 'romel@example.com']):
    globals()[f"test_invalid_id_blocks_{_idx}"] = _make_invalid_id_blocks_test(_value)

