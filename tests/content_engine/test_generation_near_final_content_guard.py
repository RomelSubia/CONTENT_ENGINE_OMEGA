...
from content_engine.content_generation_governance import validate_no_near_final_content, validate_checklist_non_creative

def test_near_final_content_blocks_publishable_examples():
    assert validate_no_near_final_content("Ejemplo de hook: texto")["status"] == "BLOCK"

def test_checklist_non_creative_only_blocks_script_like_structure():
    assert validate_checklist_non_creative(["abrir con hook", "cerrar con CTA"])["status"] == "BLOCK"
    assert validate_checklist_non_creative(["verificar policy_review", "verificar traceability_review"])["status"] == "PASS"
