...
def test_priority_passes_safe_payload():
    result = qg.calculate_priority(safe_item())
    assert result["status"] == "PASS"
    assert result["generation_allowed"] is False


def test_priority_high_does_not_enable_generation():
    item = safe_item()
    item["evidence_status"] = "COMPLETE"
    result = qg.calculate_priority(item)
    assert result["priority_band"] in {"MEDIUM", "HIGH"}
    assert result["generation_allowed"] is False
    assert qg.validate_priority_output(result)["status"] == "PASS"


def test_critical_priority_blocks():
    item = safe_item()
    item["risk_level"] = "CRITICAL_SECURITY_BLOCK"
    assert qg.calculate_priority(item)["status"] == "BLOCK"


def test_priority_validation_blocks_opened_generation():
    result = qg.calculate_priority(safe_item())
    result["generation_allowed"] = True
    assert qg.validate_priority_output(result)["status"] == "BLOCK"


def test_priority_validation_blocks_opened_queue_write():
    result = qg.calculate_priority(safe_item())
