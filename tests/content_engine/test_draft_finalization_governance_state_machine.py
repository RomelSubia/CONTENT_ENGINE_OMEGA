...
from content_engine.content_draft_finalization_governance.state_machine import (
    FinalizationState,
    assert_transition,
    can_transition,
)
...
def test_state_machine_allows_expected_transition():
    assert can_transition(FinalizationState.RECEIVED, FinalizationState.EVIDENCE_VALIDATED)

def test_state_machine_blocks_invalid_transition():
    with pytest.raises(ValueError):
        assert_transition(FinalizationState.RECEIVED, FinalizationState.READY_FOR_QUEUE_GOVERNANCE_PLAN_ONLY)
