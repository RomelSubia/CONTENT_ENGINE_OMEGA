...
from content_engine.content_generation_governance import validate_report_payload_sanitized

def test_reports_do_not_store_publishable_content():
    assert validate_report_payload_sanitized({"sample": "Ejemplo de caption"})["status"] == "BLOCK"
    assert validate_report_payload_sanitized({"risk_label": "LOW_REVIEW_BLOCK"})["status"] == "PASS"
