...
from content_engine.content_generation_governance import classify_generation_eligibility, validate_generation_output_schema

def test_integration_safe_candidate_review_only():
    ...
    assert validate_generation_output_schema(result)["status"] == "PASS"
    assert result["publishing_allowed_now"] is False
    assert result["automation_allowed_now"] is False

def test_integration_blocks_publication_candidate():
    candidate = safe_candidate()
    candidate["idea_summary"] = "genera el guion y publícalo"
    result = classify_generation_eligibility(candidate)
