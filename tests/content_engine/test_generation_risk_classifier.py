...
from content_engine.content_generation_governance import classify_generation_risk

def test_publication_precedence_over_execution():
    candidate = safe_candidate()
    candidate["idea_summary"] = "genera el guion y publícalo"
    result = classify_generation_risk(candidate)
    assert result["status"] == "BLOCK"
    assert result["risk_level"] == "CRITICAL_PUBLICATION_BLOCK"

def test_safety_risk_precedence_blocks_harmful_motivation_content():
    candidate = safe_candidate()
    candidate["idea_summary"] = "disciplina tóxica y culpa extrema"
    result = classify_generation_risk(candidate)
    assert result["status"] == "BLOCK"
    assert result["risk_level"] == "SAFETY_RISK"
