...
from content_engine.content_generation_governance import build_generation_manifest_payload, validate_generation_manifest_payload

def test_manifest_contract_validates_governance_only():
    payload = build_generation_manifest_payload(["a"], {"a": "hash"})
    assert validate_generation_manifest_payload(payload)["status"] == "PASS"
    assert payload["layer_mode"] == "GOVERNANCE_ONLY"
