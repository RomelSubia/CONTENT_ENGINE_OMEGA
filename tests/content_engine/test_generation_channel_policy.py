...
def test_channel_d_blocks_generic_motivation():
    candidate = safe_candidate()
    candidate["channel_id"] = "CHANNEL_D_AI_TECH_PERSONAL_BRAND"
    candidate["idea_summary"] = "motivación genérica"
    assert validate_channel_specific_policy(candidate)["status"] == "BLOCK"

def test_channel_a_blocks_financial_claim():
    candidate = safe_candidate()
    candidate["channel_id"] = "CHANNEL_A_MONEY_MINDSET_CONVERSION"
    candidate["idea_summary"] = "dinero garantizado"
    assert validate_channel_specific_policy(candidate)["status"] == "BLOCK"
