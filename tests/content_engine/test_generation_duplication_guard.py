...
from content_engine.content_generation_governance import validate_duplication_governance

def test_duplication_guard_does_not_read_real_history_or_files():
    candidate = safe_candidate()
    candidate["idea_summary"] = "referencia a 07_DATA"
    assert validate_duplication_governance(candidate)["status"] == "BLOCK"

def test_duplication_guard_blocks_payload_duplicate():
    candidate = safe_candidate()
    duplicate = safe_candidate()
    assert validate_duplication_governance(candidate, [duplicate])["status"] == "BLOCK"
