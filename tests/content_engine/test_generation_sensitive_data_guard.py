...
from content_engine.content_generation_governance import validate_sensitive_data_field_scope

def test_sensitive_guard_blocks_email_in_human_field():
    candidate = safe_candidate()
    candidate["idea_summary"] = "contacto blocked@example.invalid"
    assert validate_sensitive_data_field_scope(candidate)["status"] == "BLOCK"

def test_sensitive_guard_does_not_false_positive_technical_fields():
    candidate = safe_candidate()
    candidate["queue_item_id"] = "QG-20260519-121314-ABCDEF12"
    assert validate_sensitive_data_field_scope(candidate)["status"] == "PASS"
